...
class AnatomyMode:
    ...
    def __init__(self, tocabi_dir: str | None = None):
        ...
        self.tocabi_urdf: str | None = None
    ...
    def _setup_robot(self):
        ...
        self.joints = {
            ...
            "shoulder_pitch": Joint(
                "shoulder_pitch", JointType.REVOLUTE, LimbType.ARM, [0, 1, 0], -3.14, 3.14, 50.0
            ),
            "shoulder_roll": Joint(
                "shoulder_roll", JointType.REVOLUTE, LimbType.ARM, [1, 0, 0], -1.57, 1.57, 50.0
            ),
            "shoulder_yaw": Joint(
                "shoulder_yaw", JointType.REVOLUTE, LimbType.ARM, [0, 0, 1], -3.14, 3.14, 30.0
            ),
            "elbow_pitch": Joint(
                "elbow_pitch", JointType.REVOLUTE, LimbType.ARM, [0, 1, 0], 0, 2.35, 40.0
            ),
            "elbow_roll": Joint(
                "elbow_roll", JointType.REVOLUTE, LimbType.ARM, [1, 0, 0], -1.57, 1.57, 20.0
            ),
            "wrist_pitch": Joint(
                "wrist_pitch", JointType.REVOLUTE, LimbType.ARM, [0, 1, 0], -1.57, 1.57, 10.0
            ),
            "wrist_yaw": Joint(
                "wrist_yaw", JointType.REVOLUTE, LimbType.ARM, [0, 0, 1], -3.14, 3.14, 10.0
            ),
            # 5-Finger Hand
            "thumb_oppose": Joint(
                "thumb_oppose", JointType.REVOLUTE, LimbType.HAND, [0, 0, 1], -0.5, 1.5, 5.0
            ),
            "thumb_flex": Joint(
                "thumb_flex", JointType.REVOLUTE, LimbType.HAND, [0, 1, 0], 0, 1.57, 5.0
            ),
            "index_flex": Joint(
                "index_flex", JointType.REVOLUTE, LimbType.HAND, [0, 1, 0], 0, 1.57, 3.0
            ),
            "middle_flex": Joint(
                "middle_flex", JointType.REVOLUTE, LimbType.HAND, [0, 1, 0], 0, 1.57, 3.0
            ),
            "ring_flex": Joint(
                "ring_flex", JointType.REVOLUTE, LimbType.HAND, [0, 1, 0], 0, 1.57, 2.0
            ),
            "pinky_flex": Joint(
                "pinky_flex", JointType.REVOLUTE, LimbType.HAND, [0, 1, 0], 0, 1.57, 2.0
            ),
        }
    ...
    def _load_tocabi_model(self):
        ...
        if not urdf_path.exists() or not meshes_dir.exists():
            self.logger.warning(
                "tocabi_model_not_found", urdf=str(urdf_path), meshes=str(meshes_dir)
            )
            return
        ...
        try:
            ...
            for joint_elem in root.findall(".//joint"):
                ...
                child_elem.get("link") if child_elem is not None else ""
                ...
                if (
                    "hip" in name_lower
                    or "knee" in name_lower
                    or "ankle" in name_lower
                    or "foot" in name_lower
                ):
            ...
            self.logger.info(
                "tocabi_model_loaded", parts=len(self.tocabi_parts), joints=len(self.joints)
            )
    ...
    def solve_ik(
        ...
        seed_angles: dict[str, float] | None = None,
    ...
    def _forward_kinematics(self, angles: dict[str, float]) -> list[float]:
        ...
        x += (
            self.links["upper_arm"].length
            * math.cos(angles.get("shoulder_pitch", 0))
            * math.sin(angles.get("shoulder_roll", 0))
        )
        y += (
            self.links["upper_arm"].length
            * math.cos(angles.get("shoulder_pitch", 0))
            * math.cos(angles.get("shoulder_roll", 0))
        )
        return [x, y, z]
        ...
        for joint_name, angle in angles.items():
            if joint_name in self.joints:
                if not self.joints[joint_name].is_within_limits(angle):
                    violations.append(
                        f"{joint_name}: {angle:.3f} out of range "
    ...
    def to_context(self) -> str:
        return (
            f"Anatomy: {len(self.joints)} joints ({sum(1 for j in self.joints.values() if j.limb_type == LimbType.ARM)} arm, "
