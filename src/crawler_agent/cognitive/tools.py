...
class GitHubTool:
    ...
    async def _request(self, method: str, path: str, **kwargs) -> Dict:
        ...
        try:
            async with session.request(method, url, **kwargs) as resp:
                ...
                if resp.status >= 400:
                    return {"error": f"HTTP {resp.status}: {data.get('message', '')}", "status": resp.status}
                return data
    ...
    async def get_repo(self, owner: str, repo: str) -> ToolResult:
        ...
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="get_repo", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="get_repo", data=data,
            metadata={"full_name": data.get("full_name"), "stars": data.get("stargazers_count")},
            execution_time_ms=elapsed,
        )

    async def list_issues(self, owner: str, repo: str, state: str = "open", per_page: int = 30) -> ToolResult:
        start = time.perf_counter()
        data = await self._request("GET", f"/repos/{owner}/{repo}/issues", params={"state": state, "per_page": per_page})
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="list_issues", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="list_issues", data=data,
    ...
    async def get_issue(self, owner: str, repo: str, issue_number: int) -> ToolResult:
        ...
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="get_issue", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="get_issue", data=data,
            metadata={"number": data.get("number"), "title": data.get("title")},
            execution_time_ms=elapsed,
        )

    async def create_issue(self, owner: str, repo: str, title: str, body: str = "", labels: List[str] = None) -> ToolResult:
        start = time.perf_counter()
        ...
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="create_issue", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="create_issue", data=data,
            metadata={"number": data.get("number"), "url": data.get("html_url")},
            execution_time_ms=elapsed,
        )

    async def list_pulls(self, owner: str, repo: str, state: str = "open", per_page: int = 30) -> ToolResult:
        start = time.perf_counter()
        data = await self._request("GET", f"/repos/{owner}/{repo}/pulls", params={"state": state, "per_page": per_page})
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="list_pulls", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="list_pulls", data=data,
            metadata={"count": len(data), "state": state},
            execution_time_ms=elapsed,
        )

    async def get_file_content(self, owner: str, repo: str, path: str, ref: str = "main") -> ToolResult:
        start = time.perf_counter()
        import base64
        data = await self._request("GET", f"/repos/{owner}/{repo}/contents/{path}", params={"ref": ref})
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="get_file_content", error=data["error"], execution_time_ms=elapsed)
        content = ""
        if isinstance(data, dict) and data.get("content"):
            content = base64.b64decode(data["content"]).decode("utf-8", errors="replace")
        return ToolResult(
            success=True, tool="github", operation="get_file_content", data=content,
    ...
    async def search_code(self, query: str, per_page: int = 30) -> ToolResult:
        ...
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="search_code", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="search_code", data=data,
    ...
    async def list_workflow_runs(self, owner: str, repo: str, per_page: int = 30) -> ToolResult:
        start = time.perf_counter()
        data = await self._request("GET", f"/repos/{owner}/{repo}/actions/runs", params={"per_page": per_page})
        elapsed = (time.perf_counter() - start) * 1000
        if "error" in data:
            return ToolResult(success=False, tool="github", operation="list_workflow_runs", error=data["error"], execution_time_ms=elapsed)
        return ToolResult(
            success=True, tool="github", operation="list_workflow_runs", data=data,
            metadata={"count": data.get("total_count", 0)},
            execution_time_ms=elapsed,
        )
...
class ArxivTool:
    ...
    async def search(self, query: str, max_results: int = 20, sort_by: str = "relevance", sort_order: str = "descending") -> ToolResult:
        start = time.perf_counter()
        params = {
            "search_query": query,
        ...
        try:
            async with session.get(self.base_url, params=params) as resp:
                ...
                elapsed = (time.perf_counter() - start) * 1000
                return ToolResult(
                    success=True, tool="arxiv", operation="search", data=papers,
                    metadata={"query": query, "count": len(papers)},
                    execution_time_ms=elapsed,
                )
        except Exception as e:
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(success=False, tool="arxiv", operation="search", error=str(e), execution_time_ms=elapsed)

    async def get_paper(self, paper_id: str) -> ToolResult:
        start = time.perf_counter()
        session = await self._get_session()
        try:
            async with session.get(self.base_url, params={"id_list": paper_id, "max_results": 1}) as resp:
                text = await resp.text()
                papers = self._parse_arxiv_xml(text)
                elapsed = (time.perf_counter() - start) * 1000
                if papers:
                    return ToolResult(
                        success=True, tool="arxiv", operation="get_paper", data=papers[0],
                        metadata={"paper_id": paper_id},
                        execution_time_ms=elapsed,
                    )
                return ToolResult(success=False, tool="arxiv", operation="get_paper", error="Paper not found", execution_time_ms=elapsed)
        except Exception as e:
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(success=False, tool="arxiv", operation="get_paper", error=str(e), execution_time_ms=elapsed)

    async def get_recent(self, category: str = "cs.AI", max_results: int = 20) -> ToolResult:
        query = f"cat:{category}"
        return await self.search(query, max_results=max_results, sort_by="submittedDate", sort_order="descending")
...
class ROS2Bridge:
    ...
    async def publish(self, topic: str, message_type: str, data: Dict) -> ToolResult:
        if not self._available:
            return ToolResult(success=False, tool="ros2", operation="publish", error="ROS2 not available")
        start = time.perf_counter()
        try:
            ...
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(
                success=True, tool="ros2", operation="publish", data={"topic": topic},
                metadata={"message_type": message_type},
                execution_time_ms=elapsed,
            )
        except Exception as e:
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(success=False, tool="ros2", operation="publish", error=str(e), execution_time_ms=elapsed)

    async def subscribe(self, topic: str, message_type: str, callback, timeout: float = 5.0) -> ToolResult:
        if not self._available:
            return ToolResult(success=False, tool="ros2", operation="subscribe", error="ROS2 not available")
        start = time.perf_counter()
        received_data = []
        try:
            from rclpy.qos import QoSProfile
            ...
            sub = self._node.create_subscription(type(msg_module()), topic, _callback, QoSProfile(depth=10))
            # Spin for timeout
            ...
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(
                success=True, tool="ros2", operation="subscribe",
            ...
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(success=False, tool="ros2", operation="subscribe", error=str(e), execution_time_ms=elapsed)

    async def call_service(self, service_name: str, service_type: str, request: Dict) -> ToolResult:
        if not self._available:
            return ToolResult(success=False, tool="ros2", operation="call_service", error="ROS2 not available")
        start = time.perf_counter()
        try:
            srv_module = self._import_srv_type(service_type)
            client = self._node.create_client(srv_module.Request, service_name)
            if not client.wait_for_service(timeout_sec=5.0):
                return ToolResult(success=False, tool="ros2", operation="call_service", error="Service not available")
            ...
            if future.result() is not None:
                return ToolResult(
                    success=True, tool="ros2", operation="call_service",
                    data=self._msg_to_dict(future.result()),
                    execution_time_ms=elapsed,
                )
            return ToolResult(success=False, tool="ros2", operation="call_service", error="Service call failed", execution_time_ms=elapsed)
        except Exception as e:
            elapsed = (time.perf_counter() - start) * 1000
            return ToolResult(success=False, tool="ros2", operation="call_service", error=str(e), execution_time_ms=elapsed)
            ...
            for key, value in msg.__dict__.items():
                if not key.startswith('_'):
                    if hasattr(value, '__dict__'):
                        ...
                        result[key] = [self._msg_to_dict(v) if hasattr(v, '__dict__') else v for v in value]
                    else:
                        result[key] = value
            return result
        return {"data": str(msg)}
...
class ToolManager:
    ...
    async def initialize(self):
        ...
        self.tools["github"] = GitHubTool(token=github_token, api_url=api_url, proxy_pool=self._proxy_pool)
    ...
    async def invoke(self, tool_name: str, operation: str, **kwargs) -> ToolResult:
        ...
        if not tool:
            return ToolResult(success=False, tool=tool_name, operation=operation, error=f"Tool '{tool_name}' not found")
        method = getattr(tool, operation, None)
        if not method:
            return ToolResult(success=False, tool=tool_name, operation=operation, error=f"Operation '{operation}' not found")
        try:
