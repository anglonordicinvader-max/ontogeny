        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if any(self.model in name for name in model_names):
                    ...
                    self.logger.info(
                        "vision_model_not_found", model=self.model, available=model_names[:5]
                    )
        except Exception:
...
class MultimodalProcessor:
    ...
    async def process_image(
        ...
        return await self.vision.analyze_image(
            image_path=image_path, image_bytes=image_bytes, prompt=prompt
        )
    ...
    async def process_audio(
        ...
        if analyze:
            return await self.audio.analyze_audio(
                audio_path=audio_path, audio_bytes=audio_bytes, prompt=prompt
            )
        return await self.audio.transcribe(
            audio_path=audio_path, audio_bytes=audio_bytes, language=language
        )
