...
class TestModelEvaluator:
    ...
    def test_evaluate_model(self):
        ...
        results = asyncio.run(evaluator._evaluate_model(base, "base", tasks))
    ...
    def test_compare(self):
        ...
        report = asyncio.run(evaluator.compare(tasks=tasks))
...
class TestRollbackManager:
    def test_should_rollback_on_major_regression(self):
        ...
        mgr.models.append(
            ModelState(name="maldoror:v1", version="v1", adapter_path="y", active=True)
        )
...
class TestABTestRunner:
    def test_run_test(self):
        ...
        result = asyncio.run(runner.run_test(prompt="Fix the bug", num_rounds=2))
    ...
    def test_stats(self):
        ...
        asyncio.run(runner.run_test(prompt="test", num_rounds=1))
...
class TestIntegration:
    def test_full_evaluation_flow(self):
        ...
        mgr = CustomModelManager(model_trainer=t)
        mgr.models.append(
            ModelState(name="maldoror:v0", version="v0", adapter_path="x", active=True)
        )
        rb = RollbackManager(custom_model_manager=mgr)
        ab = ABTestRunner(evaluator=evaluator)

        # Run evaluation
        report = asyncio.run(evaluator.compare(tasks=evaluator.BENCHMARK_TASKS[:2]))
        assert report.verdict in ("maldoror_wins", "base_wins", "tie")
        ...
        # Run A/B test
        ab_result = asyncio.run(ab.run_test(prompt="test prompt", num_rounds=1))
